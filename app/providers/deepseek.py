    ...
    async def chat_completions(
        ...
        **kwargs: Any
        ...
        try:
            ...
            raise ProviderTimeoutError(f"DeepSeek request timed out after {self.timeout}s")
        except httpx.HTTPStatusError as e:
            if e.response.status_code == 429:
                raise ProviderRateLimitError("DeepSeek rate limit exceeded")
            raise ProviderError(f"DeepSeek API error: {e.response.status_code} - {e.response.text}")
